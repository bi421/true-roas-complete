...
def translate_to_business_action(
    ...
    capital_bleed_usd = daily_spend * max(0, (meta_roas - posterior_roas) / max(meta_roas, 0.1))

    # 5. CFO Brief Construction
    cfo_brief = (
        f"Meta reports ROAS {meta_roas:.1f}, but verified bank data shows True ROAS is {posterior_roas:.1f}. "
        f"{'Action Required: Reduce daily spend by 40% immediately to protect margins.' if status == 'BLEEDING' else 'Status Healthy: Monitor closely.'}"
    )
