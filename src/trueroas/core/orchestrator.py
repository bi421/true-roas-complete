...
from src.trueroas.core.decision_intelligence import (
    QualityEngine, ReadinessEngine, EconomicEngine, BotGuardEngine, GrowthEngine, RecommendationEngine
)
...
class DecisionContext(BaseModel):
    ...
    platform_roas: float = Field(..., ge=0, description="ROAS as reported by the ad platform")
    verified_roas: float = Field(..., ge=0, description="Verified ROAS from actual revenue")
    spend: float = Field(..., ge=0, description="Total ad spend in currency")
    revenue: float = Field(..., ge=0, description="Total verified revenue")
    sample_size: int = Field(..., ge=0, description="Order count used for statistical weight")
    ...
    inventory_days: int = Field(default=30, ge=0, description="Estimated stock remaining in days")
    vertical: str = Field(default="default", description="Business category for benchmarking")
    scale_factor: float = Field(default=1.5, ge=0.1, le=5.0, description="FTC Safeguard: Limits budget spikes")
    ...
    async def execute_audit(self, ctx: DecisionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            with PIPELINE_STAGE_LATENCY.labels(stage="traffic_safety").time():
                bot_info = BotGuardEngine.analyze_bot_risk(ctx.ctr, ctx.cr, ctx.frequency)
            ...
            DECISION_SCORES.labels(tenant_id=ctx.tenant_id, metric_type="quality").set(quality["score"])
            ...
            with PIPELINE_STAGE_LATENCY.labels(stage="bayesian_reconciliation").time():
                ...
                adjusted_verified = ctx.verified_roas * (0.5 if not bot_info["is_clean"] else 1.0)
            ...
            with PIPELINE_STAGE_LATENCY.labels(stage="readiness_evaluation").time():
                readiness = ReadinessEngine.evaluate(
                    ctr=ctx.ctr, cr=ctx.cr, freq=ctx.frequency,
            ...
            DECISION_SCORES.labels(tenant_id=ctx.tenant_id, metric_type="readiness").set(readiness["readiness_score"])
            
            bottlenecks = GrowthEngine.detect_bottleneck(ctx.ctr, ctx.cr, ctx.frequency)
            ...
            with PIPELINE_STAGE_LATENCY.labels(stage="economic_analysis").time():
                ...
                ev = (posterior["reconciled_roas"] - 1.0) * proposed_increase * readiness["readiness_score"]
            ...
            DECISION_SCORES.labels(tenant_id=ctx.tenant_id, metric_type="expected_value").set(ev)
            ...
            with PIPELINE_STAGE_LATENCY.labels(stage="recommendation_logic").time():
                ...
                effective_p_success = readiness["readiness_score"] if posterior.get("is_stable") else 0.0
                action = RecommendationEngine.determine_action(
                    p_success=effective_p_success,
                    ev=ev,
                    proposed_increase=proposed_increase,
                    safety_buffer=readiness["readiness_score"] if posterior.get("is_stable") else 0.0
                )
            ...
            with PIPELINE_STAGE_LATENCY.labels(stage="narrative_generation").time():
                return RecommendationEngine.build_defensible_advice(
                    ...
                    quality=quality, readiness=readiness, economics=economics,
                    ev=ev, action=action, reasoning=bot_info["risk_level"],
                    bot_info=bot_info, monthly_spend=ctx.spend * 30,
                    variance_pct=ctx.variance
                )
        except Exception as e:
            PIPELINE_ERRORS.labels(tenant_id=ctx.tenant_id, error_type=type(e).__name__).inc()
            logger.error(f"Decision Pipeline failed for {ctx.campaign_id}: {str(e)}", exc_info=True)
            raise