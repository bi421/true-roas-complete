...
def test_full_accountability_lifecycle_automated(sandbox_tenant):
    ...
    with duckdb.connect(db_path) as con:
        ...
        for i in range(100):
            con.execute("INSERT INTO orders (id, platform, amount, currency, created_at) VALUES (?, 'shopify', 500.0, 'USD', CURRENT_TIMESTAMP - INTERVAL '91 days')", [f"order_{i}"])
    ...
    print(f"✅ 30-day Bayesian weight verified. Risk-adjusted ROAS: {metrics['risk_adjusted_roas']}x")

    # f) 90 Days Later: Audit trail CSV export & checksum
    admin_token = generate_token(tenant_id, role="admin")
    admin_headers = {"Authorization": f"Bearer {admin_token}"}
    
    export_resp = client.get("/api/v1/export/detailed-audit-csv?days=90", headers=admin_headers)
