...
class PDFService:
    ...
    def generate_report(self, tenant_id: str, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise FileNotFoundError("Strategy PDF template 'audit_report.html' not found in templates directory.")
        ...
        if not str(tenant_reports_dir).startswith(str(storage_root)):
             raise PermissionError(f"Security: Blocked unauthorized path access for tenant: {tenant_id}")
        ...
        HTML(string=html_content, base_url=str(settings.BASE_DIR)).write_pdf(target=str(report_file_path))
