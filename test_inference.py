...
def test_conjugate_prior_logic_breakdown():
    ...
    mean, std = DecisionEngine.calculate_bayesian_posterior(meta_roas, true_roas, std_dev, sample_size)
...
def test_evidence_weighting_with_sample_size():
    ...
    mean, _ = DecisionEngine.calculate_bayesian_posterior(meta_roas, true_roas, std_dev, sample_size)
...
def test_volatility_impact_on_posterior():
    ...
    # Mean = (4*1 + 2*4) / 5 = 2.4
    mean_low_vol, _ = DecisionEngine.calculate_bayesian_posterior(meta_roas, true_roas, 1.0, sample_size)
    
    # In high volatility (std_dev = 2.0):
    # Data Precision = 4 / 2^2 = 1.0. Total Precision = 2.0.
    # Mean = (4*1 + 2*1) / 2 = 3.0
    mean_high_vol, _ = DecisionEngine.calculate_bayesian_posterior(meta_roas, true_roas, 2.0, sample_size)
