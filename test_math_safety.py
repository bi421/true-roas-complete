...
def test_calculate_posterior_stability(
    platform_roas, verified_roas, sample_size, variance, days_since_click
):
    """
...
def test_variance_inflation_luxury_goods():
    ...
    result = engine.calculate_posterior(
        platform_roas=5.0, verified_roas=1.1, sample_size=100, variance=2500000.0
    )
    assert math.isfinite(result["reconciled_roas"])
...
def test_extreme_lag_decay():
    ...
    result = engine.calculate_posterior(
        5.0, 4.0, 100, 1.0, platform="meta", days_since_click=30
    )
